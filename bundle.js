...
//	e.preventDefault();
//	var changeY = e.changedTouches[0].clientY - positionY;
//	console.log(changeY);
//	if(changeY<0){
//		index++;
//	}
...
//	}
//	$("#sections").css({
//		"transform" : "translateY(" + -winH*index+"px)"
//	});
//	console.log(index)
//});

var ScrollPage = (function(){
	function ScrollPage(element, options){
		this.element = element;
		this.settings = $.extend(true, $.fn.ScrollPage.defaults, options||{});
		this.init();
	}
	...
	ScrollPage.prototype = {
		...
		_initEvent : function(){
			...
			self.element.on("touchend", function(e){
				e.preventDefault();
				var changeY = e.changedTouches[0].clientY - touchPosY;
				if(changeY < 0){
					self.next();
				}
				if(changeY > 0){
		...
		_scrollAnimate : function(){
			...
			setTimeout(function(){
				self.section.eq(self.index).siblings().children().addClass("hide");
			},500);
			self.section.eq(self.index).children().removeClass("hide");
...
$.fn.ScrollPage = function(options){
	return this.each(function(){
		var self = $(this),
		instance = self.data("ScrollPage");
		
		if(!instance){
			self.data("ScrollPage", (instance = new ScrollPage(self, options)));
		}
